...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!recipientId.length) {
    return json({ error: 'Recipient cannot be empty' }, { status: 400 });
  }
...
export default function KudoModal() {
  ...
  return (
    <Modal isOpen={true} className="w-2/3 p-10">
      {formError && (
        <div className="text-sm font-semibold text-center tracking-wide text-red-500 w-full mb-2">
          {formError}
        </div>
      )}
      <Form method="post">
        ...
        <div className="flex flex-col items-center md:flex-row gap-x-24 gap-y-2 md:gap-y-0">
          ...
          <button
            type="submit"
            className="rounded-xl bg-yellow-300 w-80 h-12 text-blue-900 font-semibold transition duration-300 ease-in-out hover:bg-yellow-400 hover:-translate-y-1"
          >
            Send
          </button>
        </div>
